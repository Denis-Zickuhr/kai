...
| `required` | bool | Any type (default `false`, opt-in). Shows a small `*` next to the label, blocks the run form's OK button until filled, and counts toward a group's pending-required badge. Not automatic for every non-optional field — set it explicitly on the parameters that genuinely need it. No effect when `optional` is `true`. |
