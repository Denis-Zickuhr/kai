...
signals:
    ...
    // Emitido sempre que uma linha JSON ESTRUTURADA com nível de erro
    // (error/fatal/crit — mesmo predicado de colorForLevel) é adicionada.
    // Não sabe nada de "primeira vez" nem de notificação — isso é decisão
    // de quem escuta (ver OutputPanel).
...
signals:
    // Repassa LogLineModel::errorLineDetected — dono desta view (OutputPanel)
    // não precisa conhecer logModel() só pra escutar isto.
    void errorLineDetected();
