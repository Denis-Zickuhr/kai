...
void configureTable(QTableWidget *table, const QList<TableColumnSpec> &columns)
{
    ...
    // Altura mínima para caber PELO MENOS 3 linhas por padrão (feedback do
    // usuário: tabelas de envs/params/alvos apertadas demais). header + 3
    // linhas + um respiro para a moldura/scrollbar horizontal.
    const int headerH = table->horizontalHeader()->sizeHint().height();
    table->setMinimumHeight(headerH + 3 * standardRowHeight() + tk::space(3));
}
