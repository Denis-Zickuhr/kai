...
// --- Indicador visual de linha ARRASTÁVEL (drag-to-reorder) ---
// Pedido do usuário (bug relatado: "a ordenação de params ficou bem
// bugada, coloque um indicativo visual que dá pra reordenar"): sem
// nenhuma pista visual, nada na linha sugeria que ela podia ser
// arrastada — o reorder "funcionava" (drag em qualquer parte da linha,
// via DraggableTableWidget/QAbstractItemView::InternalMove), mas
// parecia quebrado por falta de affordance. Célula ícone-only (⣿ estilo
// "grip", cursor de arrastar, tooltip explicando) — não tem lógica de
// drag PRÓPRIA (a tabela inteira já trata isso), é só o indicativo
// visual, então cabe em QUALQUER DraggableTableWidget com o mesmo
// tratamento.
QWidget *makeDragHandleCell(QWidget *parent);
// Largura da coluna do indicativo de arrastar (ícone + respiro mínimo —
// mais estreita que as de ação, não é clicável).
