...
// Navegação lateral + stack de páginas no padrão visual do SettingsDialog
// (mesmo objectName "settingsNav" no QListWidget, pro QSS ficar idêntico) —
// extraído daqui para ser reaproveitado por QUALQUER diálogo re-skinado
// nesse formato (CommandEditorDialog e FolderEditorDialog, pedido do
// usuário: "aplique a mesma lógica para criação de pastas") em vez de cada
// um manter sua PRÓPRIA cópia do mesmo bloco de setup, que divergiria com
// o tempo (foi exatamente o que quase aconteceu: a versão do Command
// Editor já tinha o fix de largura mínima do nav que a de Pasta ainda não
// teria, se copiada por fora).
struct SidebarTabsHost {
    QListWidget *nav = nullptr;
    QStackedWidget *pages = nullptr;
};

// `navWidth`: largura fixa do nav. Itens mais longos que isso elidem com
// "..." (o item ganha um tooltip com o texto completo em
// addSidebarTabPage) — pedido do usuário é que os RÓTULOS das abas sejam
// curtos o bastante pra caber, não que o nav fique gigante.
inline SidebarTabsHost buildSidebarTabsHost(QWidget *parent, int navWidth = 200)
{
    auto *bodyLayout = new QHBoxLayout(parent);
    bodyLayout->setContentsMargins(0, 0, 0, 0);
    bodyLayout->setSpacing(0);

    SidebarTabsHost host;
    host.nav = new QListWidget(parent);
    host.nav->setObjectName(QStringLiteral("settingsNav"));
    host.nav->setFrameShape(QFrame::NoFrame);
    host.nav->setFixedWidth(navWidth);
    host.nav->setIconSize(QSize(16, 16));
    host.nav->setVerticalScrollBarPolicy(Qt::ScrollBarAsNeeded);
    host.nav->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
    bodyLayout->addWidget(host.nav, 0);

    host.pages = new QStackedWidget(parent);
    host.pages->setObjectName(QStringLiteral("sidebarTabsPages"));
    bodyLayout->addWidget(host.pages, 1);
    QObject::connect(host.nav, &QListWidget::currentRowChanged, host.pages, &QStackedWidget::setCurrentIndex);
    return host;
}

// Cria uma aba nova dentro de um SidebarTabsHost: página com scroll PRÓPRIO
// (viewport já protegido do bug de cascata QSS — regra qualificada por
// objectName, nunca um "background: transparent;" cru direto no viewport;
// ver histórico em CommandEditorDialog) + item correspondente na
// navegação, com tooltip do título completo (defesa contra elisão em
// larguras pequenas). Devolve o QVBoxLayout onde os cards desta aba
// entram.
inline QVBoxLayout *addSidebarTabPage(SidebarTabsHost &host, const QString &title, const QString &iconName)
{
    auto *pageContent = new QWidget();
    pageContent->setObjectName(QStringLiteral("sidebarTabPageContent"));
    pageContent->setStyleSheet(QStringLiteral(
        "QWidget#sidebarTabPageContent { background: transparent; }"));
    auto *pageLayout = new QVBoxLayout(pageContent);
    pageLayout->setContentsMargins(16, 16, 16, 12);
    pageLayout->setSpacing(12);

    auto *pageScroll = new QScrollArea(host.pages);
    ...
    pageScroll->viewport()->setObjectName(QStringLiteral("sidebarTabPageViewport"));
    pageScroll->viewport()->setStyleSheet(QStringLiteral(
        "QWidget#sidebarTabPageViewport { background: transparent; }"));

    host.pages->addWidget(pageScroll);
    auto *item = new QListWidgetItem(
        LucideIcons::icon(iconName, QColor(utils::tokens::mutedFg()), 16), title, host.nav);
...
inline QLabel *makeHintBanner(QWidget *parent, const QString &text)
{
    ...
    // Ignored na horizontal: sem isto, o sizeHint() do QLabel (baseado no
    // texto SEM quebra) ainda "vaza" pro layout pai como uma exigência de
    // largura mínima, mesmo com wordWrap ligado — Qt só respeita a quebra
    // de linha de verdade quando o label pode encolher além do seu
    // sizeHint natural. Bug real reportado com foto: o texto deste banner
    // (o mais longo do app, "Efeitos visuais" em Aparência) estourava a
    // largura do diálogo inteiro e cortava no meio da palavra, forçando
    // até uma barra de rolagem horizontal na aba — em vez de só quebrar
    // linha como deveria.
