    ...
    , m_terminalProfiles(terminalProfiles)
{
    ...
    // pro card de Identificação (grid 2 colunas) + o sidebar de abas (pedido
    // do usuário: "aplique a mesma lógica para criação de pastas"), sem
    // ficar gigante à toa — a pasta tem bem menos conteúdo que um comando.
    // 880, não 760: o sidebar (200px fixos) come da largura que antes era
    // TODA do grid de 2 colunas — sem compensar, os campos da direita
    // (Ícone/Ordem) clipavam pra fora do card (bug real visto ao testar).
    resize(880, 600);
                                  ...
                                  const QString &suggestedParentId)
{
    ...
    // Sidebar de abas (pedido do usuário: "aplique a mesma lógica para
    // criação de pastas" — CommandEditorDialog já tinha esse tratamento,
    // pasta ainda não). Reaproveita o MESMO helper (dialog-utils.h),
    // mesmo padrão visual do SettingsDialog. Duas abas: "Geral"
    // (Identificação) e "Variáveis" (env_vars) — cada uma com seu próprio
    // QScrollArea, então uma pasta com muitas env_vars não empurra os
    // campos de identificação pra fora de vista, e vice-versa.
    auto *body = new QWidget(this);
    SidebarTabsHost tabsHost = buildSidebarTabsHost(body);
    auto *generalLayout = addSidebarTabPage(tabsHost,
        utils::tr(QStringLiteral("command.tab.general")), QStringLiteral("settings"));
    ...
    // Variáveis de Ambiente (env_vars) — aba PRÓPRIA no sidebar (mesmo
    // tratamento de Headers/Extractors no Command Editor: um card sozinho
    // na própria página, já expandido, sem precisar do collapse já que
    // não compete por espaço com mais nada na aba). Cabeçalho com
    // chevron/título/badge de contagem/botão "+Adicionar", corpo com
    // estado vazio OU a tabela conforme a contagem. O editor em si
    // (KeyValueEditorWidget) não muda por dentro.
    auto *envLayout = addSidebarTabPage(tabsHost,
        utils::tr(QStringLiteral("folder.tab.env")), QStringLiteral("variable"));
    const int envNavRow = tabsHost.nav->count() - 1;
    // Contador no item de nav (pedido do usuário: "adicione contador ao
    // icone da abinha na tela de pastas e cmds") — sufixo "(N)" só com
    // N>0, mesmo padrão do Command Editor.
    auto bindEnvNavItemCount = [nav = tabsHost.nav, envNavRow](int count) {
        if (QListWidgetItem *item = nav->item(envNavRow)) {
            const QString baseTitle = utils::tr(QStringLiteral("folder.tab.env"));
            item->setText(count > 0 ? QStringLiteral("%1 (%2)").arg(baseTitle).arg(count) : baseTitle);
        }
    };
    m_envVarsCard = new CollapsibleSectionCard(utils::tr(QStringLiteral("folder.group.env")), this);
    ...
    connect(m_envVarsEditor, &KeyValueEditorWidget::changed, this, [this, bindEnvNavItemCount]() {
        const int count = m_envVarsEditor->values().size();
        m_envVarsCard->setCount(count);
        bindEnvNavItemCount(count);
    });
    if (existingFolder) {
        m_envVarsEditor->setValues(existingFolder->envVars);
    }
    m_envVarsCard->setCount(m_envVarsEditor->values().size());
    bindEnvNavItemCount(m_envVarsEditor->values().size());
    m_envVarsCard->setExpanded(true, false); // sozinho na própria aba — sem motivo pra nascer colapsado
    m_envVarsCard->setCollapsible(false); // pedido do usuário: não precisa mais colapsar
    ...
    // Aba "Geral" selecionada por padrão (mesmo padrão do Command Editor,
    // ver command-editor-dialog.cpp) — faltava aqui: o QStackedWidget já
    // mostrava a página 0 por padrão (primeiro widget adicionado), mas o
    // QListWidget do nav nunca recebia currentRow(0), então nenhum item
    // aparecia MARCADO/destacado mesmo com o conteúdo certo visível
    // (relatado: "a aba de GERAL não fica visualmente marcada, mesmo
    // estando selecionada").
    tabsHost.nav->setCurrentRow(0);
