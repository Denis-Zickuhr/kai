    ...
    : QWidget(parent)
{
    ...
    if (!logoPixmap.isNull()) {
        ...
        logoWidget->setStyleSheet(QStringLiteral(
            "background-color: %1; color: white; border-radius: %2px; font-size: %3pt; font-weight: 700;")
            .arg(utils::tokens::accent()).arg(utils::tokens::radiusMd()).arg(utils::tokens::fontSizeTitlePt()));
    }
...
QWidget *WelcomeScreen::buildProTipCard()
{
    ...
    tint.setAlphaF(0.08);
    card->setStyleSheet(QStringLiteral(
        "QWidget#welcomeProTip { background-color: rgba(%1,%2,%3,%4); border: 1px solid %5; border-radius: %6px; }")
